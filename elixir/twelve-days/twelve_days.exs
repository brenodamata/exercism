defmodule TwelveDays do
  ...
  def verse(number) when number < 13 and number > 0 do
    case number do
  ...
  def verse(_), do: raise ArgumentError, message: "Give me a number from 1 - 12"
  ...
  def verses(starting_verse, ending_verse) do
    get_verses(starting_verse, ending_verse, [])
  end

  defp get_verses(start, endd, ret) when start < endd+1 do
    new_verse = verse(start)
    get_verses(start + 1, endd, ret ++ [new_verse])
  end

  defp get_verses(_, _, ret), do: ret |> Enum.join("\n")
